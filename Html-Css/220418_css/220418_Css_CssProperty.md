...
- 색의 표현(CSS의 전체에서 사용가능)
  ...
  - hsla : 색상, 채도, 명도, 투명도 (hsl(120, 100%, 50%, 10% ))
...
|  background-position  | 요소 배경의 이미지 위치           |              0% 0%               | top, bottom, left, right, center (값을 입력하면 앞은 x축, 뒤의 값은 y축 값이 적용된다.)            |
| background-attachment | 배경 이미지 스크롤 특성           |              scroll              | fixed(이미지가 뷰포트에 고정, 스크롤 x)                                                            |
...
- transition : css가 변화가 있을 때, 에니메이션 속도 조절하는 방법을 제공합니다.
  - 속성명/지속시간(필수)/타이밍함수/대기시간
  - 기본 : all
